...
from code2flow.ruby import Ruby
from code2flow.php import PHP
...
def make_file_group(tree, filename, extension):
    ...
    file_group = Group(token, group_type, display_name, import_tokens,
                       line_number, parent=None, path=filename)
...
def map_it(sources, extension, no_trimming, exclude_namespaces, exclude_functions,
    ...
    fix_module_naming_collision(file_groups)
...
def fix_module_naming_collision(file_groups):
    # check if any file_group.name is the same as any other file_group.name
    tokens = [fg.token for fg in file_groups]
    if len(tokens) != len(set(tokens)):
        # Fixing the naming collision by adding longer module path to tokens
        duplicate_file_groups = find_duplicate_file_groups(file_groups)
        for file_group in duplicate_file_groups:
            file_group.token = dedup_token(file_group.path)
        fix_module_naming_collision(file_groups)

def find_duplicate_file_groups(file_groups):
    token_counts = collections.defaultdict(list)
    for fg in file_groups:
        token_counts[fg.token].append(fg)
    return [fg for fgs in token_counts.values() if len(fgs) > 1 for fg in fgs]

def dedup_token(path):
    path_clean = os.path.splitext(path)[0]
    tokens = os.path.normpath(path_clean).split(os.sep)[-2:]
    return ".".join(tokens)
